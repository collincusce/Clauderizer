...
HOST_EMITTERS: dict[str, HostEmitter] = {
    ...
    # Kimi Code CLI (the successor to the legacy Kimi CLI; serves Kimi K3): its
    # MCP registration is a project-level JSON config identical in shape to
    # Cursor's — {"mcpServers": {command,args}} at .kimi-code/mcp.json — so it is
    # a first-class auto-write host (D-049, applying D-031's project-config
    # branch). Session-start hooks live in ~/.kimi-code/config.toml [[hooks]]
    # (TOML) → guide-only; the reconciled kimi-setup.md carries that snippet.
    "kimi":       HostEmitter("kimi", ".kimi-code/mcp.json", "mcpServers", True,
                              "Kimi Code CLI project MCP config; hooks are guide-only "
                              "TOML in ~/.kimi-code/config.toml (see .clauderizer/kimi-setup.md)"),
...
def configure_hints(host_id: str) -> list[str]:
    ...
    hints: dict[str, list[str]] = {
        ...
        "kimi": [
            "MCP is auto-written to `.kimi-code/mcp.json` — nothing to do.",
            "Session-start hooks are guide-only (TOML) — merge the [[hooks]] snippet "
            "from `.clauderizer/kimi-setup.md` into `~/.kimi-code/config.toml`.",
            "Skills: Kimi Code CLI reads `.kimi-code/skills`/`.agents/skills`, not "
            "`.claude/skills` — see `.clauderizer/kimi-setup.md` §4.",
        ],
        "windsurf": [
...
# Kimi Code CLI hook events whose stdout reaches context (exit 0 → stdout added
# to context, verified 2026-07-16 moonshotai.github.io/kimi-code hooks docs). Of
# its 13 events these four carry the status digest; one portable command serves
# all four (the engine dispatches on hook_event_name and stays silent otherwise).
KIMI_HOOK_EVENTS = ("SessionStart", "UserPromptSubmit", "PreCompact", "PostCompact")

# Portable, machine-independent kimi hook command (no wsl.exe, no absolute path).
KIMI_HOOK_COMMAND = ["uvx", "--from", "clauderizer", "clauderizer-hook"]


def kimi_setup_guide(hook_argv: list[str] | None = None) -> str:
    """The Kimi Code CLI setup guide written to .clauderizer/kimi-setup.md (D-049).

    Kimi Code CLI (the successor to the legacy Kimi CLI; the host that serves Kimi
    K3) auto-gets its MCP registration via .kimi-code/mcp.json (written for real by
    emit_mcp). This guide covers the two pieces that stay manual: the session-start
    hooks (TOML in ~/.kimi-code/config.toml — the zero-runtime-dep invariant forbids
    a stdlib TOML rewrite) and skills exposure (Kimi Code CLI reads .kimi-code/skills
    and .agents/skills, NOT .claude/skills). TOML literal strings (single quotes)
    carry the command verbatim so Windows backslashes are not read as escapes."""
    hook_cmd = " ".join(hook_argv or KIMI_HOOK_COMMAND)
    mcp_cmd = " ".join(PORTABLE_COMMAND)
    hook_blocks = "\n\n".join(
        f"[[hooks]]\nevent = \"{ev}\"\ncommand = '{hook_cmd}'"
        for ev in KIMI_HOOK_EVENTS
    )
    return f"""# Clauderizer × Kimi Code CLI setup
...
def emit_host_wiring(host_id: str, repo_root: Path) -> list[EmitResult]:
    ...
    if host_id == "grok":
        ...
        # MCP is auto-written above; the guide covers hooks (TOML, guide-only) and
        # skills exposure (Kimi Code CLI does not read .claude/skills) — D-049.
        guide_path = repo_root / ".clauderizer" / "kimi-setup.md"
        results.append(EmitResult(
            "hook-guide", guide_path,
            _write_text_if_changed(guide_path, kimi_setup_guide())))
    else:
