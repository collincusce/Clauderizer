...
The agentic tool surface: it exposes the markdown memory as **31 self-describing MCP tools** over stdio, so an agent dropped into a clauderized repo discovers the entire workflow from the tool list alone — no reading order, no doc pass. Each tool's name, signature, and docstring carry its schema, and that is the whole onboarding.
...
- **Reads** (`writes=False`, 6) — side-effect-free by contract (L-03), never take the write lock: `cz_status`, `cz_next_phase_context`, `cz_graph_query`, `cz_analyze`, `cz_critique`, `cz_mine_failures`.
- **Rituals** — `cz_preflight` (runs host test/build, refreshes the tracked baseline), `cz_cascade` / `cz_resolve_cascade` (walk dependents, then record verdicts), `cz_write_handoff`.
- **Mutations** — entity/status (`cz_upsert_entity`, `cz_transition_status`), the memory writes (`cz_add_decision`, `cz_add_invariant`, `cz_add_finding`/`cz_resolve_finding`, `cz_add_lesson` and the lesson-curation set `cz_consolidate_lessons`/`cz_obsolete_lesson`/`cz_promote_lesson`, `cz_add_correction`), the Ending-Protocol writes (`cz_add_output`, `cz_add_phase_summary`), and gameplan structure (`cz_create_gameplan`, `cz_add_phase`, `cz_transition_phase`, `cz_add_amendment`).
