...
- **Wiring contract (CI-gated, automatable):** emitted config is valid JSON, carries a well-formed `clauderizer` entry (`command` present), is path-safe (no machine-specific absolute paths), and names `clauderizer-mcp` in its argv. Since 1.14.0 it also **launches**: `hosttargets.verify_emitted_wiring` spawns the emitted command and completes an MCP `initialize` handshake asserting `serverInfo.name`, so a config naming a command that does not exist now FAILS the contract (it previously passed for all eleven auto-write hosts — H-20). An unreachable target reports `unverifiable` and does not fail the gate, because a false red mirrors the false green this closed. The remaining manual part is unchanged (D-032): whether the real host actually READS the config and injects it is still an irreducibly manual spot-check.
