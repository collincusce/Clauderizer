...
### Planning context (2026-07-09)

- **Repo**: `/home/ccusce/Clauderizer` (dogfood; engine 1.5.3).
- **Focus**: this gameplan (stole focus from standing curator loop — restore when done).
- **Why**: Grok Build TUI can run Clauderizer MCP + AGENTS.md floor today, but has no
  first-class `host-target`. SessionStart hooks *run* on Grok; **stdout is not model
  context** → governance hooks, not Tier-1. Never add `grok` to `_HOOK_HOSTS`.
- **Primary code**: `hosttargets.py`, `session.py` (`_HOOK_HOSTS` / `_PROMPT_HOSTS` /
  P7 bootstrap), `scaffold/init.py`, `cli.py` doctor, `docs/CROSS-HOST.md`.
- **Live proof already**: viderizer Grok session used `cz_*` via MCP; no cold digest.
- **MCP note**: this Grok session's MCP server may still be cwd-bound to another repo —
  for Clauderizer writes use `cd ~/Clauderizer && clauderize ops`.
- **First work**: Phase 0 is docs/measurement only — matrix row draft, O-01/O-02,
  no production Tier-1 claims.
