...
- [ ] Gap-detection leg (A-002/D-075): gap-conversion rate measured — fraction of gap events followed within the same gameplan by a recorded decision/lesson overlapping the gap's phase — plus gap-rate trend as the corpus matures; a null result is a successful outcome (L-50)
- [ ] Reinforce-verb leg (A-002/D-075): corpus TOKEN weight (H-26's metric, never entry count) and per-lesson re-derivation rate measured before/after the verb exists — verdict cites both figures
- [ ] jcode-host row (A-002/D-075): verified by live capability probe or recorded as a named gap; if verified, jcode joins the non-Claude-host legs above
- [ ] Per-mechanism verdict written for ALL adopted mechanisms — the ten Fractal-vetted (D-070) plus the four jcode-vetted (D-075) — graduate as default / documented-dormant / withdraw, each with its pre-named signal's measured figure (L-50) — recorded as decisions/amendments; discards documented as successful outcomes
