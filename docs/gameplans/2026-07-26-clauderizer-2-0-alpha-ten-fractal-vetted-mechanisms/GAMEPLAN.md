...
### A-002 — jcode-vetted mechanisms join the alpha: gap detection, reinforce verb, negative-space close-outs, jcode-host verification (new Phase 8) + phase-5 legs

- **Date**: 2026-07-28
- **Affected sections in GAMEPLAN.md**: Phases (new Phase 8 "jcode-vetted attention mechanisms" — depends on Attention and consolidation (P3) for the advisory/telemetry seams it extends; executes any time after P3, before Phase 5's matrix); Phase 5 exit criteria (three legs added: gap-conversion signal, reinforcement re-derivation signal, jcode-host verification row); Phase 7 (the fleet briefing contract gains the negative-space completion-report clause); ENFORCEMENT.md rows for the new mechanisms come from whichever of Phase 4 / Phase 8 lands second (the A-001 ordering rule, extended); new artifact research-jcode-vetting.json (this dir)
- **Affected phases**: 5, 7, 8 (new)
- **Triggered by**: User direction 2026-07-28: deep-research github.com/1jehuang/jcode, vet techniques against our recorded law, add phases; explore "build in this direction or another project which works with this". Vetting recorded in research-jcode-vetting.json under the phase-0 methodology; adoption decision D-075.
- **What changed**: Adds Phase 8 carrying the four D-075 mechanisms under their binding conditions: (1) memory-gap detection (cz_analyze gap advisory at the moment of the gap + text-free telemetry gap events + corpus_health count; tool-results only, never a digest line); (2) reinforce-instead-of-duplicate (third verb on the write-time near-duplicate advisory: blessed reinforcement trailer on the existing lesson via the single grammar-safe renderer + telemetry event; strength as curator evidence, never authority); (3) negative-space close-outs (what-I-did-not-check in fleet completion reports — hub-enforced by judgment, engine detector explicitly deferred-unenforced — plus procedure text; the fleet-contract clause rides Phase 7's productization); (4) jcode-host verification (live capability probe of the documented compat surfaces — repo-root .mcp.json, AGENTS.md, .claude/skills — before any wiring claim; env markers captured from a real session before any detect arm; no native emitter unless evidence demands). Phase 5 gains the pre-named signals as matrix legs. Rejected-with-routing recorded in the vetting JSON: embedding/ambient evidence to semantic-recall, selfdev-channel evidence to workflow-critique's engine-identity phase, swarm/overnight stay harness scope, multi-harness mining deferred pending real fixtures.
- **Why**: jcode is the same problem attacked from the harness side, and its enforcement stack independently converges on this repo's law — which makes its few genuinely novel mechanisms unusually cheap to translate: gap detection closes a real hole (nothing here records when memory had nothing to say; jcode's own version logs-and-drops, so ours exceeds the source), the reinforce verb is the missing cheap alternative to consolidation that H-26 showed making handoffs heavier, negative-space declarations are the honest complement to deferred-status honesty, and the host verification is the lowest-cost concretization of the user's "works with this" direction. Everything numeric jcode claims was stripped (benchmark data lives outside their repo); adoption rests on code evidence per D-070's bar, advisory-first per INVARIANT-05, matrix-gated per D-064.
...
**O-01.** _(phase Live state and budgets)_ Recording coverage gates budget meaning: before reserve-window-wind-down can graduate, measure the fraction of real sessions that produce recorded spend stints (per host kind). If coverage is low, wind-down math is fiction — the phase-5 verdict must cite the measured number and choose dormant/withdraw accordingly. (Vetting condition: "measure recording coverage FIRST since it gates everything.") _(resolved 2026-07-28: Measured FIRST per the vetting condition: 0 stint events across 134 telemetry records vs 6 real working sessions since the recorder landed (P2 cabbd3e) — coverage 0.0, all sessions claude-code. Root cause is the serving-engine capability gap (H-30: session MCP = published 1.14.3, which predates record_run_stint), NOT agent non-compliance (every phase ran cz_preflight). Recorder capability proven live on the tree engine: ops-mode leg recorded a stint + derived wind_down with the phase-aware advisory; a real kimi CLI session recorded a stint via CLI ops (kimi_pinned_budgets leg). Full figures in matrix-p5-results.json (recording_coverage + budgets_ops_mode + kimi_pinned_budgets legs). Budgets verdict accordingly: documented-dormant with a post-publish re-measure gate, not withdraw)_

**O-02.** _(phase Evidence matrix and graduation)_ Matrix harness legs required by the vetting conditions — kimi-pinned, ops-mode, ≥1 non-Claude host, slow-FS WSL row, an under-adhering host — need confirmed availability/harness before phase 5 begins. A leg that cannot run becomes a named gap in that mechanism's graduation verdict, never a silent skip. _(resolved 2026-07-28: Availability confirmed leg-by-leg before execution; every leg either RAN or is a NAMED GAP with its blocking reason (matrix-p5-results.json leg_availability(O-02)): claude-code ran (hub + 6 spawned agents); kimi CLI 1.49.0 ran headless (--print) as both the kimi-pinned budgets arm and a non-Claude-host leg; ops-mode ran; slow-FS WSL ran BOTH filesystems (ext4 + DrvFs); jcode = honestly-unverifiable row recorded by P8 (no binary/toolchain/credential, recipe recorded); live Cursor leg = named gap (GUI launcher only, cursor-agent CLI absent); live under-adhering host = named gap (kimi adhered; the backstop's seeded-fixture leg covers the shape such a host leaves, fire/quiet geometry complete))_
...
**Exit criteria**:
- [x] Recording coverage measured FIRST and recorded with figures — the budgets verdict cites it explicitly (low coverage ⇒ wind-down math is fiction ⇒ dormant or withdraw)
- [x] Matrix legs executed and logged per condition: Claude Code + >=1 non-Claude host; kimi-pinned + ops-mode legs for budgets; slow-FS WSL row for the stamp (stamped vs unstamped arms); an under-adhering host for the backstop detector; any leg that cannot run is a named gap in the verdict, never a silent skip
- [x] Stranded-state: zero false positives across same-host-MCP-stranding vs A-alive and cross-host controls, or the mechanism stays advisory-dormant — verdict recorded with counts
- [x] Backstop detector fires on seeded abandoned work and stays quiet on healthy closes across the matrix; never-engaged advisory fires on a genuinely ignored finding and stays quiet on engaged ones — both with counts
- [x] Merge audit seeded-fault protocol run: detection rate on true merges, clean-merge false-positive rate, and healthy-repo digest byte-identity all recorded
- [x] Fleet-vs-solo leg run per D-071's pre-named signal: the same seeded task solo vs an N-worker fleet (hub-and-spoke, cz_assign partitioning) — exit-criteria pass quality, wall-clock, LockHeld/collision counts, and independent-verifier-found defects recorded; the 'more agents = better' hypothesis confirmed, refuted, or bounded, and the clauderizer-fleet skill's guidance updated to match the figures
- [x] Gap-detection leg (A-002/D-075): gap-conversion rate measured — fraction of gap events followed within the same gameplan by a recorded decision/lesson overlapping the gap's phase — plus gap-rate trend as the corpus matures; a null result is a successful outcome (L-50)
- [x] Reinforce-verb leg (A-002/D-075): corpus TOKEN weight (H-26's metric, never entry count) and per-lesson re-derivation rate measured before/after the verb exists — verdict cites both figures
- [x] jcode-host row (A-002/D-075): verified by live capability probe or recorded as a named gap; if verified, jcode joins the non-Claude-host legs above
- [x] Per-mechanism verdict written for ALL adopted mechanisms — the ten Fractal-vetted (D-070) plus the four jcode-vetted (D-075) — graduate as default / documented-dormant / withdraw, each with its pre-named signal's measured figure (L-50) — recorded as decisions/amendments; discards documented as successful outcomes
