...
### State handed off by Phase 0 (commit 184c353)

- Baseline: **1379 passed, 7 skipped**. Procedure version **1.10.0**.
- The `deferred` prerequisite is LIVE: `cz_transition_phase(..., "deferred",
  reason=...)` writes `⏸️ DEFERRED — <reason>`; aliases exited/abandoned/
  superseded/wontfix map into it; tracker headers mirror `_lifecycle`.
  The judgment menu's "close honestly" disposition is therefore writable.
- Binding conditions: `../research-fractal-vetting.json` keys
  `heal-on-proof-stranded-state` and `intent-postmortem-with-backstop-landings`.

### Phase 1 traps from the refuter verdicts

- **POSIX-gate the probe**: on win32, `os.kill(pid, 0)` sends
  CTRL_C_EVENT/TerminateProcess — an ungated probe can interrupt a live
  process from a read path. Non-posix grades `inconclusive`, never probes.
- **Display-only contract**: the detect/read path writes ZERO bytes under
  docs/ and .clauderizer/ (sessions.jsonl untouched on read) — state it in
  the module docstring (rituals/memory_lag.py style) and prove it by test.
- **Adopt re-stamps evidence** via the blessed same-status `in_progress`
  transition (`_set_phase_row` documents same-status as a healing touch).
- **Backstop fire-quiet geometry** errs SILENT: fire only when phase
  in_progress AND >=1 non-docs commit since the tracker anchor AND every
  closing residue absent; any post-anchor handoff refresh or phase summary
  keeps it quiet (INVARIANT-08).
- **One voice (L-55/L-65)**: the fired record's describe() must subsume the
  memory_lag wording AND explain clean_tree's dirty-tree FAIL and do-phase's
  STOP step — do not add a fourth phrasing. Reuse memory_lag's bounded
  `_git` (10s timeout) pattern for any git evidence.
- **Refusal journal is separable** — ship or defer independently; if built,
  the ok:False wrapper sits at Op/REGISTRY construction so it covers BOTH
  run_op (CLI) and the MCP server's direct REGISTRY path.
- Everything advisory; no recommended-default claims before the phase-5
  matrix (D-064).
