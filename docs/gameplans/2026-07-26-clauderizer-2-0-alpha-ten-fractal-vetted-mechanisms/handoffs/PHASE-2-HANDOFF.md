...
### State handed off by Phase 1 (commit 586d572)

- Baseline: **1424 passed, 7 skipped**. Procedure 1.10.0. Phases 0-1 complete.
- Binding conditions: `../research-fractal-vetting.json` keys
  `per-call-live-state-stamp` and `reserve-window-wind-down`. C-01 records
  the liveness-gate delta the matrix must respect.

### Phase 2 ORDER IS BINDING (exit criterion 1)

1. **FIRST** record the INVARIANT-08 amendment as an append-only supersession
   (figures-only, change-triggered structured notices on tool results defined
   as a category DISTINCT from status injection) — the amendment's entry must
   predate the first stamp commit in git history. Use cz_add_invariant with
   supersedes=INVARIANT-08 (or the house supersession pattern) + a D-NNN.
2. Only then the cz_state stamp: there is ALREADY a `_stamped` contract
   wrapper at ops.py REGISTRY construction (~line 1561) and now a `_journaled`
   one — the state stamp becomes a third wrapper at the same seam.
   Change-triggered: in-memory last-emission (module state in ops.py is fine —
   the MCP server process is the session); attach `cz_state` only when the
   figure set or revision differs. Wrapper isolation BOTH directions pinned;
   figures key-whitelist ratchet test; exit-criteria figure must skip/byte-cap
   approval artifact_hash recompute (status_bundle.py artifact hashing).
3. Budgets: two tiers ONLY (gameplan-sessions, phase-sessions); spend unit =
   distinct-DATE stints from telemetry (host-stable; proc tag tiebreak only);
   WIND_DOWN derived at read time, module constant reserve fraction, no
   flags, no persistence; phase-aware wording ("IN the final budgeted stint —
   land the Ending Protocol" vs "recorded spend exceeds budget"); pre_compact
   convergence surfacing ships IN this phase (L-68 clause 1); ships DORMANT
   (no filled template default). O-01 (recording coverage) stays open for
   phase 5 unless measured here.

### Useful evidence sources already in place

- `.clauderizer/telemetry.jsonl` outcome/surfaced records carry dates —
  the distinct-DATE stint substrate. `cz_revision` exists (revision counter).
- Session ledger stamps (P1) carry `at` dates per phase claim — corroborating
  stint evidence, but telemetry is the primary (recorded-never-estimated).
