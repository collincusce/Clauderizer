...
### Source-of-truth values (captured at plan time, 2026-07-26)

- Baseline: **1253 tests** green; version **1.14.4** (dev); plan commit parent `8766411`.
- Research provenance: workflow `wf_cb73fc8e-315` (29/29 agents); fractal pinned at `b591353`.
- **Binding conditions live in `../research-fractal-vetting.json`** — per D-070 the
  `vetted[].verdict.conditions` entries are binding; `vetted[].assessment` is the grounded
  starting sketch (file:line cites were verified against src/ on 2026-07-26 but re-verify
  line numbers before editing).

### Phase 0 scope = two mechanisms (artifact keys)

- `honest-terminal-vocabulary` — deferred as the honest terminal door. Known loci from the
  assessment: `mutations.py` transition_phase (~1121–1133, 1227+: `_PHASE_DISPLAY`/aliases),
  `_refresh_tracker_headers` vs `status_bundle._lifecycle` divergence (COMPLETE+DEFERRED
  renders 'Executing'), reason sanitization (splitlines()[0] IndexError on empty reason —
  fix in the sketch), dual skill copies (src asset + installed render, L-16/L-65), ops.py
  docstring + MCP tools_list schema. CHANGELOG note owed: pass_rate becomes goal-met rate.
- `unknowable-never-zero-epistemics` — four residual conflation sites; the frame is
  "finish enforcing D-065/D-069 house law", not new capability. Write the missing-arm tests
  FIRST (`conditions.py` TimeoutExpired/OSError arms have zero coverage — the test IS the
  detector). `met` stays boolean + additive `unevaluable`; working-time disclosure is the
  severable weakest quarter — ship hedged ('~N active day(s)', None on any git failure).

### Traps the refuters flagged for THIS phase

- Deferred must NOT nag about unchecked exit criteria (it is the honest door — nagging
  recreates the laundering incentive). Advisory has no flag (INVARIANT-05).
- No fourth status: ABANDONED/EXITED are aliases INTO deferred; a process-death status has
  no writer in a memory layer (that detection belongs to rituals/memory_lag.py).
- The corpus sweep must assert flips move only toward open/deferred — a flip toward
  complete is the laundering direction and means the matcher is wrong.
- Digest must stay byte-identical on a healthy repo (INVARIANT-08 golden test).
