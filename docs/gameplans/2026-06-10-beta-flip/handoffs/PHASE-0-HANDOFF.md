...
- `docs/gameplans/2026-06-10-beta-flip/GAMEPLAN.md`
- `docs/gameplans/2026-06-10-beta-flip/PHASE-STATUS.md`
- `CLAUDE.md`
...
**Phase 0 = the burn-down (D2).** GP-C of the beta arc (D-012, B6); GP-A and
GP-B evidence + scopes live in their gameplan dirs (post-mortems + outputs
registries; the refined GP-C scope is in stranger-readiness'
`b5_consolidated_and_gpc_scope`).

**The must-have: the bare-IO meta-test.** A structural test (suggested home:
tests/test_structural_writes.py or a new tests/test_io_discipline.py) that
walks src/ and tests/ asserting no `read_text(` / `write_text(` without
`encoding=` and no text-mode `open(` without encoding (allowlist: "rb"/"wb"
modes, this meta-test itself). The cp1252 class it guards bit 35 call sites
in B2. Prove it fires (L-10): assert it catches a synthetic violation
(e.g. scan a tmp file's text, or a deliberately-excluded fixture string).
Implementation note: simple regex over Path.rglob("*.py") source text is
fine — this is a tripwire, not a parser; keep allowlisting explicit.

**Best-effort (defer without ceremony per D2):**
- MCP-staleness nudge: cz_status digest line when any src/clauderizer mtime
  > the server process start time (server records start at import; the
  status bundle compares). One ⚠ line, engine-side.
- Doc-sync check: release-check marker-grep that README's release section
  names `release-check` (the G7-drift-between-sibling-docs tripwire).

**Captured source-of-truth (2026-06-10, all verified this session):**
- pyproject.toml: version "0.9.0" (line 7), classifier
  "Development Status :: 3 - Alpha" (line 15 — Phase 1 flips it).
- Suite 264 (260 pass + 4 interop-skip on the linux leg). 13 project
  lessons. B1–B5 ✅ in docs/RELEASING.md's evidence table.
- CHANGELOG [Unreleased] already covers the whole B1–B5 arc — Phase 1
  retitles it [0.10.0].
- quickstart.yml's self-arming step is keyed on `if [ "$V" = "0.9.0" ]` —
  the flip release arms it automatically; Phase 2 cites the armed run.
- H-01..H-09 all resolved (B6 precondition: zero open findings at flip).
- Release lanes: phases 0–2 touch no workflow files → WSL-side git push
  throughout; gh WSL-side for the Release; editable reinstall after the
  version bump (H-03); post-release release-check exit 2 on the SHIPPED
  version is designed (it goes exit-0 only pre-tag on the new number).
- The wiring fix being shipped: _under_uv_cache + `uvx -q --from
  clauderizer` durable form (src/clauderizer/scaffold/init.py, live-proven
  via local wheel in stranger-readiness Phase 0).
