...
**Why this gameplan exists.** Borrow three discipline gates from GitHub's spec-kit
(see D-014's neighbor analysis) into the Clauderizer engine: (Phase 0) a *clarify*
gate = structured open items; (Phase 1) machine-checkable *exit criteria*; (Phase 2)
an *analyze* gate = surface conflicting invariants/decisions for agent judgment;
(Phase 3) integration + docs.

**The cross-cutting mandate (applies to every phase) — D-015 / D-016 / INVARIANT-05.**
The gates are ALWAYS-ON, ADVISORY, and JUDGMENT-BASED. They surface findings in the
tool result and let the agent rule; they MUST NOT hard-block a mutation or phase
transition, and they MUST NOT add an enable/disable `[rituals]` config flag. User
directive verbatim: *"avoid configs, we should be intelligent about it."* The agent
is the control surface. The model to copy is `cz_cascade`: "the engine finds and
reports; it does not decide." Reuse existing markdown conventions, no new schemas (D1).

---

### Phase 0 — Clarify gate: structured open items (O-NN)

**What exists today (verified 2026-06-17 via codebase map):**
- The GAMEPLAN template already has a prose `## Open Items` section
  (`src/clauderizer/templates/gameplan/GAMEPLAN.md`, ~L28–30: `_(O1, O2, … —
  blockers and cross-phase questions.)_`). Real gameplans render it.
- NO structured tracking: no auto-numbering, no resolved state, no `cz_status`
  reporting, no surfacing at phase transition. It is prose only.

**What to build:**
- `cz_add_open_item(gameplan_id="", text, phase=None)` → append an auto-numbered
  **O-NN** entry to the "Open Items" section. Model exactly on
  `mutations.add_decision` (the D-NNN allocator). Returns `{ok, id, path,
  files_changed}`; idempotent.
- `cz_resolve_open_item(gameplan_id="", id, resolution)` → mark O-NN resolved in
  place (a `**Resolved:** …` line, or move to a "Resolved" subsection). Idempotent.
- `cz_status` → report the count of UNRESOLVED open items for the active gameplan
  (extend the status bundle + the SessionStart digest line).
- `cz_transition_phase` → when transitioning, SURFACE unresolved open items relevant
  to that phase in the result (advisory, NO block — honors INVARIANT-05). Keep the
  relevance heuristic trivial here (items tagged to the phase, else all gameplan-level);
  the analyze gate (Phase 2) is where relevance gets clever.

**Establish the shared "advisory surfacing" result shape here** (Phases 1 & 2 reuse
it — their goals say "reuses Phase 0's surfacing convention"). Suggestion: results
carry an optional `advisories: [{kind, ids, message}]` field so transition can report
unresolved open items now, unchecked exit criteria (Phase 1), and candidate conflicts
(Phase 2) uniformly. Lock the shape in Phase 0.

**ID convention call:** use dashed **O-NN** (width 2) for consistency with D-NNN /
L-NN / INVARIANT-NN blessed IDs, and update the template prose hint ("O1, O2") to
match. `model.next_numbered_id(text, "O", sep="-", width=2)`.

**Source-of-truth anchors (file:line, from the 2026-06-17 map):**
- Op registry (add both tools, `writes=True`): `src/clauderizer/ops.py:416–441`;
  MCP registration loop `src/clauderizer/mcp_server.py:65–66`; CLI batch
  `src/clauderizer/cli.py` `cmd_ops` (~L275) → `ops.run_batch`.
- Tool-name parity: `src/clauderizer/tools_list.py` (`TOOL_NAMES`) +
  `tests/test_blessed_surfaces.py` (parity is enforced — update both or the test fails).
- Mutations: `src/clauderizer/mutations.py` — `add_decision` (~L102–153) is the
  template for `add_open_item`; `transition_phase` (~L705–754) is the surfacing hook;
  `@_locked` wrapper (~L37–52).
- Writers: `src/clauderizer/markdown/writer.py` — `append_to_section` (~L73–77),
  `set_labeled_value` (~L86–99), `upsert_section`.
- ID allocator: `src/clauderizer/model.py` `next_numbered_id` (~L140–170).
- Template: `src/clauderizer/templates/gameplan/GAMEPLAN.md`.
- Tests: `tests/test_mutations.py` (write-op pattern), `tests/conftest.py`
  (`temp_repo` ~L16–20). Dominant pattern = write → read → idempotent-rewrite (L-01).
- DO NOT add a preflight enable/disable check (INVARIANT-05) — surfacing lives in the
  `transition_phase` result + `cz_status`, always on. Preflight loop is at
  `src/clauderizer/rituals/preflight.py:144–252` for reference only.

**Baseline:** suite 266 (preflight-measured 2026-06-17); the map counted 267 test
functions — `cz_preflight` will re-measure at phase start.

**Phase 0 exit criteria (verifiable):**
- [ ] `cz_add_open_item` + `cz_resolve_open_item` on the registry, reachable via BOTH
      MCP and `clauderize ops`; `test_blessed_surfaces` green.
- [ ] O-NN auto-numbered; add + resolve are idempotent (write → read → rewrite tests).
- [ ] `cz_status` reports the unresolved-open-items count for the active gameplan.
- [ ] `cz_transition_phase` surfaces unresolved relevant open items in its result
      WITHOUT blocking (advisory; honors INVARIANT-05); covered by a test.
- [ ] A reusable advisory-surfacing result shape is established for Phases 1–2.
- [ ] Zero new config flags (INVARIANT-05); suite green (baseline + new tests).
