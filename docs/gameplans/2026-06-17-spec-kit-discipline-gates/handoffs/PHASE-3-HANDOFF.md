...
**This is the capstone — no new gate mechanics.** Phases 0–2 shipped all three
gates (5 new tools, the `advisories` shape, the analyze surfacing). Phase 3 WIRES
them into the workflow surfaces and DOCUMENTS them to code (claims-cite-code, G7),
then the gameplan closes.

### What's already built (verified 2026-06-17)
- Tools (29 total; parity green): `cz_add_open_item` / `cz_resolve_open_item`
  (clarify), `cz_set_exit_criteria` / `cz_check_exit_criterion` (exit criteria),
  `cz_analyze` (analyze). `cz_add_decision` enriches its result with `related`.
- `transition_phase`→complete returns `result["advisories"]` (kinds `open_items`,
  `exit_criteria`); `cz_status` bundle has `open_items` + a digest line.
- Decisions D-015 / D-016 (project), gameplan D1 / D2; INVARIANT-05; lessons #1–2.

### Phase 3 — Integration & docs
- **Digest:** open-items line exists (Phase 0). Consider adding the current/next
  phase's unchecked-exit-criteria count to `status_bundle.compute` + `render_digest`
  (optional; mirror the open-items line).
- **Skills (wire the gates into the workflow):** `.claude/skills/clauderizer-*/SKILL.md`
  — `clauderizer-do-phase` close-out should `cz_check_exit_criterion` the phase's
  criteria; `clauderizer-new-gameplan` / `clauderizer-record` should `cz_analyze`
  before recording a decision and `cz_add_open_item` for unresolved ambiguity.
- **Docs to code:** `CLAUDE.md` "Key tools" list (add the 5 new tools);
  `docs/ARCHITECTURE.md` + `docs/subsystems/mutations.md` / `rituals.md` (describe the
  gates); `CHANGELOG.md` (tool surface 24 → 29, the three gates). DECISIONS/INVARIANTS
  already carry D-015/D-016/INVARIANT-05. Cite implementation, don't narrate intent.
- **Version:** a release decision — likely just a `CHANGELOG` Unreleased entry this
  phase; the actual bump/release is its own ritual (see RELEASING.md), not required to
  close this gameplan.
- **Set + check this phase's own exit criteria** (`cz_set_exit_criteria` phase 3, then
  `cz_check_exit_criterion`) — dogfood, and complete clean.

### Close-out (this is the last phase)
After Phase 3: run `clauderizer-close-gameplan` — promote the enduring lessons
(#1 verify-the-map, #2 advisory-excludes-scaffold are good candidates), write the
POST-MORTEM, clear `active_gameplan`. Suite must be green (283 + any Phase 3 tests).

**Anchors:** `status_bundle.render_digest`/`compute`; `.claude/skills/clauderizer-*/`;
`CLAUDE.md`; `docs/ARCHITECTURE.md`, `docs/subsystems/`; `CHANGELOG.md`;
`tools_list.py` (29).
