...
restart_validation_observation: RESOLVED 2026-06-10 (second cold start): digest NO again — and root-caused as H-08. The harness DID execute the registered command (trust, timeout, and stdin all eliminated: transcript hook_non_blocking_error attachments show it ran in 383ms with hasTrustDialogAccepted=true), but Claude Code on Windows executes hook commands through Git Bash, whose MSYS2 POSIX→Windows argument conversion rewrites /bin/sh to C:/Program Files/Git/usr/bin/sh before wsl.exe spawns; Ubuntu's default-shell wrapper cannot exec the mangled path (exit 127), so the D4 wrapper — and its H-01 breadcrumb — never ran (failure below the deepest observable layer, lesson #6). Identical attachments in BOTH wrapper-era cold starts (228fb4d0 / Phase-4 session, 6b9a162f / this session). Reproduced outside the harness: 'C:\Program Files\Git\bin\bash.exe' -c "<registered command>" → exit 127, stderr byte-identical. Two MSYS-immune command shapes verified working through Git Bash: wsl.exe -d ubuntu sh -c 'exec /home/.../hook.sh' and MSYS_NO_PATHCONV=1 prefix (both need cmd/PowerShell re-validation before wiring adoption). Doctor's hook-launchability probe does not traverse the Git-Bash leg — claim gap recorded in H-08. USER DECISION 2026-06-10: proceed with the 0.8.0 release repair with H-08 open (pre-release; we are the only consumers until 1.0); the wiring fix is next-gameplan work.
...
release_0_8_0_state: REPAIR NEEDED (2026-06-10 ~04:37Z): user cut v0.8.0 from the GitHub UI while origin/main lagged at the Phase-1 commit (22ac029, source 0.6.0) — the UI binds tags to the remote branch head, so the tag/Release point at a wrong-version tree and 'Publish to PyPI' failed as a duplicate-0.6.0 (the H-07 gate exists but lives in the unpushed commits, so it could not fire). Same mechanism as v0.7.0 (then-origin-head 6f8afc1). 0.8.0 is NOT burned: PyPI rejected the upload, so the number is recoverable. Repair (next session, with user confirmation for the remote-state moves): (1) push main — origin gets a182dc1: source 0.8.0 + the gate; (2) retarget the tag: git push origin :refs/tags/v0.8.0, then tag a182dc1 (or current HEAD) as v0.8.0 and push it; (3) delete + recreate/re-publish the v0.8.0 Release with the prepared notes so the release:published event re-fires — the gate then validates tag==source and PyPI receives a real 0.8.0; (4) verify via Actions run success + uvx --refresh resolution. Local truth unchanged: source 0.8.0, suite 215 green, doctor 16/16 certifying 0.8.0. Digest restart validation still pending and still informative — check it before the remote repair.
```
