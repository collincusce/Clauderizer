...
release_0_7_0_state: pyproject + __version__ = 0.7.0; venv dist-info refreshed (pip install -e ., H-03 lesson); CHANGELOG 0.7.0 section covers all five phases; README gains 'No MCP client?' (ops), 'Split-host wiring' (host-of-record + wrapper + identity) sections, updated CLI block (--session-host, --no-spawn-test, ops verb, doctor exit codes 0/2/3) and tree (hook.sh); installed procedure synced to 1.2.1; re-init: 20/20 kept (wiring stable); doctor 16/16 exit 0 via shim certifying 0.7.0. TAG NOT CUT — gated on a restart-validated cold start; publishing fires on the GitHub Release (publish.yml Trusted Publishing), not the bare tag. Tests 215 (baseline 211 + 3 identity + 1 amendment).
