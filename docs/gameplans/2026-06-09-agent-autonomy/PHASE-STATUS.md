...
release_0_8_0_state: SHIPPED 2026-06-10: PyPI publish verified — Actions run 27256861511 success (35s, gate passed tag==source) and a force-fresh resolve confirms: uvx --refresh --from clauderizer clauderize --version → clauderizer 0.8.0. Repair executed per the e880ced plan: main pushed 22ac029..e207b63 (e207b63 = records commit: H-08 + restart-validation verdict), broken UI Release deleted with its notes preserved byte-for-byte (/tmp/v080-notes.md, 3364 bytes), remote tag v0.8.0 retargeted to e207b63 (tag == main == source 0.8.0), Release recreated from the saved notes → release:published re-fired. Push gotcha worth keeping: the WSL-side git credential AND the gh CLI token both lack the workflow scope, so any push touching .github/workflows/ is rejected from WSL — the push went through Windows git + Git Credential Manager (full grant). SSH fallback unavailable non-interactively (key is passphrase-protected; no agent holds it). Restart validation itself: digest NO — shipped anyway by explicit user decision (pre-release, sole consumers until 1.0) with H-08 open; the wiring fix is next-gameplan work.
```
