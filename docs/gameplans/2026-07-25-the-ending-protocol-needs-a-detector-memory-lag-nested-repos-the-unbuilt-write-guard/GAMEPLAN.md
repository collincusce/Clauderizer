...
### A-003 — Re-distill docs/LESSONS.md under L-26's coverage gate, on thematic overlap rather than the never-surfaced signal

- **Date**: 2026-07-25
- **Affected sections in GAMEPLAN.md**: Phase Breakdown (Phase 4 close-out), docs/LESSONS.md
- **Affected phases**: 4
- **Triggered by**: User direction after the H-25 finding: "lesson bloat, then handle dreamer notes, then ship." The digest has warned for several releases that 26 project lessons exceed the threshold and ride in every handoff across every gameplan (~11.2k tokens).
- **What changed**: Re-distill the active project-lesson corpus as part of Phase 4's close-out, following the two recorded lessons that govern this exact operation. Basis is THEMATIC synthesis per L-57 — cz_corpus_health reports 0 lexical near-duplicate pairs, so near-duplicate merging has nothing to merge and one principle drawn across several war-stories is the honest lever. Mechanics per L-57: cz_add_lesson the synthesis then cz_promote_lesson it, then cz_obsolete_lesson each source with a "consolidated into L-NN" pointer; never cz_promote_lesson a source, which is a no-op for entries already in docs/LESSONS.md. Gate per L-26: derive each source lesson's query from its OWN distinctive tokens rather than hand-picked probes, and require the synthesis to surface in the ranker top-k for every source query, proven BEFORE apply (simulated) and AFTER apply against the live file with the obsolete marker stripped so the check cannot cheat.
- **Why**: Explicitly NOT done on the never-surfaced signal, which is the lever the digest's own nudge suggests and which H-25 has just invalidated: five active lessons (L-11, L-24, L-52, L-56, L-57) have never been surfaced to anything because relevance ranking is wired to phase handoffs and not to planning, so their zero is an artifact of where the ranker is wired rather than evidence of low value. Obsoleting on that basis would delete exactly the lessons the engine never gave a chance — and would compound the defect, since L-11 is the planning lesson whose absence H-25 documents. Recording the basis explicitly because a future reader will otherwise assume the standard nudge was followed.
