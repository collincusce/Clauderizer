...
**4.** A gate that is GREEN SOMETIMES is worse than no gate, because its failure mode is training you to press re-run. The published-MCP release check piped its JSON-RPC messages and closed stdin, so the server could exit on EOF before answering; it passed on one release commit, failed on the next, and passed again on a bare re-run while the artifact was healthy the whole time. Two rules follow. Drive a protocol endpoint the way a real client does -- hold the stream open and read with a deadline -- never fire-and-close, because the pass then depends on scheduling. And when a gate goes red, DIAGNOSE before re-running: I reproduced it locally in one command, which is what separated 'the artifact is broken' from 'the check is'. Re-running first would have hidden a real defect behind a green. *(evidence: Phase 5 of the 1.14.2 backlog gameplan: .github/workflows/quickstart.yml stranger job; red on c34d56d, green on a bare re-run, published server proven at 67 tools via a stdin-held probe)*
