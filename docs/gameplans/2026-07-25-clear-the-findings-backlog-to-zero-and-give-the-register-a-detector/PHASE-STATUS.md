...
TAG_AND_RELEASE_1142: CI 10/10 at JOB granularity on the EXACT released commit 35b5268, before any tag existed -- 9 matrix cells (ubuntu|macos|windows-latest x 3.11|3.12|3.13) plus the fresh-clone leg, checked per job rather than by workflow conclusion. release-check 9/9 green. Tag v1.14.2 -> d9a018d annotated on 35b5268, pushed and confirmed on the remote via ls-remote. GitHub Release cut at https://github.com/collincusce/Clauderizer/releases/tag/v1.14.2. Ordering held: origin/main took the commit before the tag existed, the tag before the Release, and publishing fires on release:published via PyPI trusted publishing (OIDC, no stored token). Notably the tag candidate moved twice during the close -- f9f8343 was verified 10/10 but two docs-only commits followed it, so the tag went on the CURRENT head after re-verifying rather than on the earlier green, since the criterion is the exact commit being released.
PUBLISHED_AND_PROVEN_1142: publish.yml: build success, publish success, PyPI trusted publishing (OIDC, no stored token). IN-BAND upload evidence read from the job log rather than inferred from the index, which lags: 'Uploading clauderizer-1.14.2-py3-none-any.whl', 'Uploading clauderizer-1.14.2.tar.gz', then 'View at: https://pypi.org/project/clauderizer/1.14.2/'. CAPABILITY proof per L-25: plain `uvx --refresh --from clauderizer[mcp] clauderizer-mcp` from a non-repo cwd completed a real MCP initialize handshake returning serverInfo clauderizer 1.14.2 with 67 tools (cz_status / cz_dream / cz_cascade present). The probe deliberately used the CORRECTED shape this phase shipped -- stdin held open with a bounded read -- rather than the fire-and-close pipe whose race was the release-gate defect fixed in e4fb082; using the old shape to certify the fix would have been the false green in miniature.
H27_VERIFIED_ON_THE_PUBLISHED_BUILD: The deployment check H-23 taught us to make, made. Running the PUBLISHED 1.14.2 against this repo (`uvx --refresh --from clauderizer clauderize status`) emits: 'Engine identity: this process is NOT running this repo's source -- it imported clauderizer from ~/.cache/uv/archive-v0/.../site-packages/clauderizer (both report 1.14.2)'. Two things this proves that a fixture could not. The detector reaches the surface that matters now that it is published, closing the deployment gap recorded at Phase 0. And it fired in the BOTH-REPORT-THE-SAME-VERSION case -- the exact shape that made H-27 invisible during 1.14.1 -- so the design decision to disambiguate on PATH rather than version is validated in production rather than only in a test.
```
