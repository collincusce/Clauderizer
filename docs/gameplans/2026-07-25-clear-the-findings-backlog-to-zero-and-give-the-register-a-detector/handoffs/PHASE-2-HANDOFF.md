...
**L-67.** Curating append-only memory DOWN is consolidate + obsolete, gated on COVERAGE rather than taste. Mechanics: add the synthesis (cz_add_lesson → cz_promote_lesson), then cz_obsolete_lesson each source with a 'consolidated into L-NN' pointer — never delete (INVARIANT-03), and never cz_promote_lesson a source, which is a no-op for entries already in docs/LESSONS.md. cz_lesson_health's 'promotion candidate' flags a high-utility lesson to KEEP, not one to move. Basis: with 0 lexical near-duplicate pairs the honest lever is THEMATIC synthesis — one principle drawn across several war-stories — not near-duplicate merging, which has nothing to merge. Gate: auto-derive each source's query from its OWN distinctive (rarest-across-corpus) tokens rather than hand-picked probes, require the synthesis to surface in the ranker top-k for every source query, and prove coverage BOTH before apply (simulated) and after apply on the live file with the obsolete marker stripped, so the check cannot cheat by matching the bookkeeping it just wrote. Merge only the tightest clusters. Instance: 21 → 16 active at 9/9 post-apply coverage, rollup −20%. Never curate on a NEVER-SURFACED signal without first checking that the surfacing machinery could have reached that lesson at all — a zero produced by where the ranker is wired is not evidence about the lesson. (Consolidates L-26, L-57.) *(evidence: thematic re-distill under L-26's coverage gate; consolidates L-26, L-57 at 10/10 pre-apply coverage)* *(from 2026-07-25-the-ending-protocol-needs-a-detector-memory-lag-nested-repos-the-unbuilt-write-guard)*
...
**L-63.** Reject a CLASS of malformed input by a STRUCTURAL PROPERTY, never by a list of the bad values. A blocklist is incomplete by construction (every new field, error or host adds a name) and destructive at the edges (it eats legitimate content that happens to look like the thing). Find the property that separates malformed from well-formed and test THAT: leaked tool-call framing is an UNBALANCED closing tag while real markup in prose is balanced; an unindexable entity doc is one that INTENDED to be an entity (a fence, a partial id/type) while ordinary prose intended nothing. Such a predicate needs no maintenance as the surface grows and provably cannot touch well-formed input. Two riders, both learned the hard way: exempt code spans and fenced blocks, or the guard eventually rewrites the very documentation and tests that describe it (the read side already established this exemption); and classify CONSERVATIVELY at the boundary — only inputs that clearly intended to be the thing count as failures, or the report acquires standing false positives and stops being read. *(from 2026-07-25-the-ending-protocol-needs-a-detector-memory-lag-nested-repos-the-unbuilt-write-guard)*
