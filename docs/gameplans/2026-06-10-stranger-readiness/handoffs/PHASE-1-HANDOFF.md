...
**Phase 1 = upgrade + uninstall stories** (see GAMEPLAN.md Phase 1 goal;
independent of phases 2–3 per L-11). State at handoff: Phase 0 complete —
quickstart fixed and guarded (quickstart.yml green, run 27316260960), suite
264, matrix 9/9 (run 27316260956).

**Upgrade-walk ingredients (all proven this gameplan):** simulate the
pre-H-09 wrapper with `render_hook_wrapper(baked, windows=…)` WITHOUT root
(the doctor "template predates" nudge fires, exit 3); plain re-init heals
byte-idempotently. The 0.9.0→next upgrade story will ALSO heal the ephemeral
uvx wiring (Phase 0's fix changes what init composes — doctor's freshness
check flags the old baked engine line as "engine moved"). uvx-cache caveat:
upgrades via uvx need `--refresh` or they answer from cache (L-08).

**Uninstall-walk surfaces to remove (verify against scaffold/init.py):**
.mcp.json `clauderizer` key (key-merge — other servers preserved);
.claude/settings.json SessionStart entries matching hosts.is_hook_command;
.clauderizer/ directory; the CLAUDE.md marker block
(`<!-- clauderizer:start -->`…`end`); .claude/skills/clauderizer-*.
KEEP: docs/ — the project's memory, not the tool's. End-state checks:
`clauderize doctor` → "Not a clauderized repo" exit 1; a session cold start
shows no digest and no errors.

**Deliverable**: docs/UPGRADING.md (upgrade + uninstall sections), commands
executable verbatim (lesson #1: the doc is the test script — consider
extending quickstart.yml or asserting the walks in tests where cheap).
README link lands in Phase 4's pass (avoid double-editing README here).
